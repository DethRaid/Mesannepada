...
public:
    ...
    void set_from_toml(const std::unordered_map<std::string, toml::value>& table) override;
...
void CVarSystemImpl::set_from_toml(const std::unordered_map<std::string, toml::value>& table) {
    if (logger == nullptr) {
        logger = SystemInterface::get().get_logger("CVarSystem");
    }

    for(const auto& [cvar, value] : table) {
        auto* cvar_data = GetIntCVar(std::string_view{cvar});
        if(cvar_data) {
            logger->debug("Setting cvar {} to {}", cvar, value.as_integer());
            *cvar_data = static_cast<int32_t>(value.as_integer());
        } else {
            logger->warn("Unknown cvar {}, skipping", cvar);
        }
    }
}
...
void CVarSystemImpl::DrawImguiEditor() {
    ...
    //alphabetical sort
    std::sort(
        cachedEditParameters.begin(),
        cachedEditParameters.end(),
        [](const CVarParameter* a, const CVarParameter* b) {
