...
namespace render {
    ...
    void CommandBuffer::clear_texture(const TextureHandle texture) const {
        const auto is_depth = is_depth_format(texture->create_info.format);
    ...
    void CommandBuffer::set_scissor_rect(const VkRect2D rect) const {
        vkCmdSetScissor(commands, 0, 1, &rect);
    }
    ...
    void CommandBuffer::set_topology(const VkPrimitiveTopology topology) const {
        vkCmdSetPrimitiveTopology(commands, topology);
    }

    void CommandBuffer::set_fill_mode(const VkPolygonMode fill_mode) const {
        vkCmdSetPolygonModeEXT(commands, fill_mode);
    }
    ...
    void CommandBuffer::begin_label(const std::string& event_name) {
        ...
        auto labelled_zone = std::make_unique<tracy::VkCtxScope>(
            backend->get_tracy_context(),
            TracyLine,
            TracyFile,
            strlen(TracyFile),
            TracyFunction,
