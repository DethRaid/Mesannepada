...
namespace render {
    ...
    void DepthCullingPhase::render(RenderGraph& graph, const RenderWorld& world, MaterialStorage& materials,
                                   SceneView& view) {
        ZoneScoped;
        ...
        const auto visible_objects = view.get_visible_objects_buffer();
        const auto num_primitives = world.get_total_num_primitives();

        draw_visible_objects(
            graph, world, view_descriptor, masked_view_descriptor, primitive_buffer, visible_objects, num_primitives);
        ...
        view.set_visible_objects_buffer(this_frame_visible_objects);

        draw_visible_objects(
            graph, world, view_descriptor, masked_view_descriptor, primitive_buffer,
            view.get_visible_objects_buffer(), num_primitives);
    ...
    void DepthCullingPhase::create_command_signature() {
        auto& backend = RenderBackend::get();
        const auto tokens = eastl::array{VkIndirectCommandsLayoutTokenNV{
        ...
        const auto create_info =
            VkIndirectCommandsLayoutCreateInfoNV{.sType = VK_STRUCTURE_TYPE_INDIRECT_COMMANDS_LAYOUT_CREATE_INFO_NV,
    ...
    eastl::optional<BufferHandle> DepthCullingPhase::create_preprocess_buffer(const GraphicsPipelineHandle pipeline,
        ...
        if(requirements.memoryRequirements.size > 0) {
            ...
            return allocator.create_buffer(
                "Preprocess Buffer", requirements.memoryRequirements.size, BufferUsage::StorageBuffer);
        } else {
    ...
    void DepthCullingPhase::draw_visible_objects(RenderGraph& graph, const RenderWorld& world,
                                                 ...
                                                 const BufferHandle visible_objects,
                                                 const uint32_t num_primitives) const {
        // Translate the list of objects to indirect draw commands

        const auto solid_buffers =
             ...
             .buffers =
                 {
                     ...
                     {solid_buffers.count,
                      VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT,
                      VK_ACCESS_2_INDIRECT_COMMAND_READ_BIT},
                     {solid_buffers.primitive_ids, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, VK_ACCESS_2_SHADER_READ_BIT},
                     ...
                     {cutout_buffers.count,
                      VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT,
                      VK_ACCESS_2_INDIRECT_COMMAND_READ_BIT},
                     {cutout_buffers.primitive_ids, VK_PIPELINE_STAGE_VERTEX_SHADER_BIT, VK_ACCESS_2_SHADER_READ_BIT},
                 },
