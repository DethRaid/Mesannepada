...
namespace render {
    static AutoCVar_Enum cvar_ao_technique{"r.AO", "What kind of AO to use", AoTechnique::RTAO};

    static AutoCVar_Int cvar_rtao_samples{"r.AO.RTAO.SamplesPerPixel", "Number of RTAO samples per pixel", 1};
    ...
    void AmbientOcclusionPhase::generate_ao(RenderGraph& graph, const SceneView& view, const RenderWorld& world,
                                            const NoiseTexture& noise, const TextureHandle gbuffer_normals,
                                            const TextureHandle gbuffer_depth, const TextureHandle ao_out) {
        ZoneScoped;

        frame_index++;

        if(cvar_ao_technique.get() == AoTechnique::RTAO && !RenderBackend::get().supports_ray_tracing()) {
            cvar_ao_technique.set(AoTechnique::Off);
        }
        ...
        case AoTechnique::Off:
            graph.add_render_pass(
                {.name = "Clear AO",
                 .color_attachments = {RenderingAttachmentInfo{.image = ao_out,
    ...
    void AmbientOcclusionPhase::evaluate_rtao(RenderGraph& graph, const SceneView& view, const RenderWorld& world,
                                              const NoiseTexture& noise, const TextureHandle gbuffer_depth,
                                              const TextureHandle gbuffer_normals, const TextureHandle ao_out) {
        auto& backend = RenderBackend::get();
        if(rtao_pipeline == nullptr) {
            rtao_pipeline = backend.get_pipeline_cache().create_pipeline("shader://ao/rtao.comp.spv"_res);
        }

        const auto set = backend.get_transient_descriptor_allocator()
                             .build_set(rtao_pipeline, 0)
        ...
        graph.add_compute_dispatch(ComputeDispatch<Constants>{
            .name = "Ray traced ambient occlusion",
            .descriptor_sets = {set},
            .push_constants = Constants{.samples_per_pixel = static_cast<uint32_t>(cvar_rtao_samples.get()),
