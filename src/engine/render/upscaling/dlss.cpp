...
namespace render {
    static auto cvar_dlss_mode = AutoCVar_Enum{
        "r.DLSS.Mode", "DLSS mode", sl::DLSSMode::eMaxQuality
    };
    ...
    void DLSSAdapter::evaluate(
        ...
        const TextureHandle motion_vectors_in, const TextureHandle average_exposure_in,
        const TextureHandle normals_roughness
        ) {
        ...
        const auto use_dlss_rr = *CVarSystem::Get()->GetIntCVar("r.GI.Denoiser") == static_cast<uint32_t>(
                                     DenoiserType::DLSS_RR);
        ...
        graph.add_pass(
        {
            ...
            .execute = [&](CommandBuffer& commands) {
                ...
                auto motion_vectors_in_res = wrap_resource(
                    motion_vectors_in,
                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
                auto average_exposure_res = wrap_resource(
                    average_exposure_in,
                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
                ...
                if(normals_roughness != nullptr) {
                    sl_normals_roughness =
                        wrap_resource(normals_roughness, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
                    tags.emplace_back(
                        &sl_normals_roughness,
